...
The inputs are OVERRIDABLE because the machine changed. They used to be four
hard-coded paths pointing at the 4070's results, which is how the published
page came to be showing 82.8 GF/s and 75.1 ms/step while the README had been
re-measured on a 5080. A generator that cannot be pointed at the current card
guarantees the page drifts from the repo eventually -- which is the exact
failure this file was written to prevent.

usage: python3 tools/make_site_data.py [results.csv] [train.log] [sample.txt]
       python3 tools/make_site_data.py bench/results_5080.csv \\
                                       bench/logs/train_5080_tf32.log \\
                                       bench/logs/sample_5080.txt
"""
import csv
import json
import os
...
LOG = sys.argv[2] if len(sys.argv) > 2 else "bench/logs/train_final.log"
SAMPLE = sys.argv[3] if len(sys.argv) > 3 else "bench/logs/sample_best.txt"
# The TF32-baseline sweep is optional: it exists for the 4070 and the section
# it feeds is simply omitted when it does not.
...
# The headline size is PINNED to 4096, not taken as the largest size measured.
#
# It used to be max(cublas), which was 4096 for as long as the sweep stopped
# there. Adding N=6144 silently moved every headline number on the page to a
# size the README does not quote -- and moved them all UPWARDS, because bigger
# matrices amortise the tile load better: dbuffer reads 98.7% of cuBLAS at 6144
# against 87.2% at 4096, and cpasync 136% against 126%.
#
# A statistic that improves because the sweep got wider is not a result, and
# the direction it moved is exactly the direction that flatters. Both writeups
# quote the same size or they are not comparable to each other.
BIG = 4096 if 4096 in cublas else max(cublas)
kernels = []
...
if os.path.exists(SAMPLE):
    s = open(SAMPLE, encoding="utf-8", errors="replace").read()
    j = s.find("\n\n")
...
hdr = ("// GENERATED by tools/make_site_data.py -- do not edit.\n"
       f"// Numbers come from {CSV} and {LOG}.\n\n")
