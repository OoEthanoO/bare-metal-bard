@echo off
REM Sweep kernel 11's tile configs. Each one is a separate compile, because the
...
REM
REM   scripts\sweep_k11.bat [size] [cfg ...]     (default 4096, all configs)
REM
...
for %%c in (%CFGS%) do (
  "%NVCC%" %FLAGS% -DK11_CFG=%%c src\bench.cu src\registry.cu !KERNELS! ^
      -o bench\sgemm_k11.exe -lcublas >nul 2>&1
  if errorlevel 1 (
    echo cfg %%c: build failed
  ) else (
    for /f "tokens=*" %%l in ('bench\sgemm_k11.exe -k 11 -s %SIZE% -i 20 -w 5 ^| findstr /C:"cpasync"') do echo cfg %%c  %%l
  )
)
