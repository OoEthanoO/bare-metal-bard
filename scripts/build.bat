...
REM Targets: sgemm test_gemm train_gpt test_grad test_flash test_ddp test_ddp_gpt test_xent device_query
REM
...
goto argloop
:argdone
if "%TARGETS%"=="" set "TARGETS=sgemm test_gemm train_gpt test_grad test_flash test_ddp test_ddp_gpt test_xent bench_nn device_query"
...
for %%t in (%TARGETS%) do (
  ...
  if "%%t"=="test_xent"    "%NVCC%" %FLAGS% tools\test_xent.cu src\nn.cu -o bench\test_xent.exe
  if "%%t"=="test_ddp_gpt" "%NVCC%" %FLAGS% tools\test_ddp_gpt.cu %GPT_SRC% -o bench\test_ddp_gpt.exe
